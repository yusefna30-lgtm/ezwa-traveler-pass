const landmarks = [
    { id: 'murabba', name: 'قصر المربع التاريخي', lat: 24.6433, lon: 46.7111, radius: 5.0 },
    { id: 'masmak', name: 'قصر المصمك', lat: 24.6318, lon: 46.7126, radius: 5.0 },
    { id: 'diriyah', name: 'حي الطريف التاريخي بالدرعية', lat: 24.7350, lon: 46.5744, radius: 5.0 },
    { id: 'hegra', name: 'موقع الحِجر التاريخي بالعلا', lat: 26.7886, lon: 37.9515, radius: 10.0 },
    { id: 'oldtown', name: 'بلدة العلا العتيقة', lat: 26.6131, lon: 37.9231, radius: 10.0 }
];
...
function unlockVault(serial) {
    ...
    // محاولة فحص الـ GPS الحقيقي أولاً
...
function checkRealGeoLocation() {
    if ("geolocation" in navigator) {
        navigator.geolocation.getCurrentPosition(function(position) {
            ...
            // في حال رفض إذن الموقع، تبقى الأختام مقفلة حتى يستخدم أزرار المحاكاة
        });
    }
}

// وظيفة محاكاة الموقع التجريبي للأزرار
function simulateLocation(lat, lon, landmarkName) {
    processCoordinates(lat, lon);
    let statusBadge = document.getElementById("auth-status");
    statusBadge.innerHTML = `[وضع المحاكاة] تم رصد الموقع في: ${landmarkName}`;
}
...
function processCoordinates(userLat, userLon) {
    landmarks.forEach(landmark => {
        ...
        if (distance <= landmark.radius) {
            ...
            storyText.innerText = `أهلاً بك في ${landmark.name}. تم تسجيل الزيارة بنجاح في الأرشيف الميداني لقطعتك [ EZWA ].`;
        }
    });
}
